...
import asyncio
import sys
from intellishell.main import IntentShell
...
from intellishell.utils.clipboard import should_pipe_to_clipboard, copy_to_clipboard
from intellishell.utils.display import format_message
from intellishell import __version__
import logging
...
async def execute_single_command(
    ...
    # Handle ambiguous
    from intellishell.parser import AmbiguousMatch, IntentMatch
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  intellishell                     Start interactive shell
  intellishell -c "open desktop"   Execute single command
  intellishell --debug             Start with debug output
  intellishell --no-ai             Disable AI fallback
  intellishell --no-memory         Disable semantic memory
        """
    )
    ...
    parser.add_argument(
        "-v", "--version",
        action="version",
        version=f"IntelliShell v{__version__}",
    )
    ...
    logger.info(f"IntelliShell v{__version__} starting")
    ...
    try:
        if args.command:
            ...
            from intellishell.main import main as repl_main
            repl_main(
                debug=args.debug,
                enable_ai=(not args.no_ai),
